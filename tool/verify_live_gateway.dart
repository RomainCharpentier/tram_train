...
Future<void> main() async {
  ...
  final gateway = SncfGateway(httpClient: httpClient, apiKey: apiKey, mapper: mapper);
  ...
  print('📅 Requested local datetime: $requested (weekday=${requested.weekday})');

  try {
    final trains = await gateway.findJourneysWithDepartureTime(from, to, requested);
    if (trains.isEmpty) {
    ...
    final preview = trains.take(5).map((t) =>
        '${t.departureTime.toIso8601String()} (wd=${t.departureTime.weekday})').join('\n');
